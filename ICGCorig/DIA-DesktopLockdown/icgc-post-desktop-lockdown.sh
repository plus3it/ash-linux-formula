...
AFFIL_VERSION="2011.3-12.dia.v1.0.dia.desktop"
...
function doEnableFingerprint {

## Commented this routine in main on 10 Sept 2013 due to authconfig modifying files in /etc/pam.d
## post configuration

	# Re-enabling fingerprint (was disabled for all non-desktop flavors)
	writeToLog "----------------------------------------------------------"
	writeToLog "14c) Calling - doEnableFingerprint - Re-enable fingerprint"
	/usr/sbin/authconfig --enablefingerprint --update 2>&1 | tee -a $LOGFILE
	writeToLog " "
...
echo "-- START: Disabling last logon display on console" 2>&1 | tee -a $LOGFILE

if [ ! -d /etc/gconf/gconf.xml.mandatory ]; then
  echo " " 2>&1 | tee -a $LOGFILE
  echo "  WARNING...could not find /etc/gconf/gconf.xml.mandatory!" \
  2>&1 | tee -a $LOGFILE
  echo " " 2>&1 | tee -a $LOGFILE
fi

/usr/bin/gconftool-2 --config-source xml:readwrite:/etc/gconf/gconf.xml.defaults \
--direct --type bool --set /apps/gdm/simple-greeter/disable_user_list true

echo "-- END: Disabling last logon display on console" 2>&1 | tee -a $LOGFILE

#---------------------------------------------------
# Non-audit
# Set screen saver to 15 minute timeout

echo "-- START: Set screen saver to 15 minute timeout" 2>&1 | tee -a $LOGFILE
...
echo "-- END: Set screen saver to 15 minute timeout" 2>&1 | tee -a $LOGFILE
...
echo "-- START: Disabling CTL-ALT-DEL using gconftool-2" 2>&1 | tee -a $LOGFILE

/usr/bin/gconftool-2 --direct \
--config-source xml:readwrite:/etc/gconf/gconf.xml.defaults \
--type string --set /apps/gnome_settings_daemon/keybindings/power 'none'
...
echo "-- END: Disabling CTL-ALT-DEL using gconftool-2" 2>&1 | tee -a $LOGFILE
...
#
# Note: To be sure the line is properly configured, it will be removed
# and re-added if present

echo "-- START: Disabling root login on console" 2>&1 | tee -a $LOGFILE

cp -f /etc/pam.d/gdm-password /etc/pam.d/.gdm-password.$CURDATE 2>&1 | tee -a $LOGFILE

PAM_RT_LINE="pam_succeed_if.so"

if [ "`grep -e \"$PAM_RT_LINE\" /etc/pam.d/gdm-password`" ]; then
  #echo "Found pam_succeed_if.so, stripping out and replacing"
  cat /etc/pam.d/gdm-password |grep -v $PAM_RT_LINE  > /etc/pam.d/gdm-password.tmp
  printf "auth\trequired\tpam_succeed_if.so user != root quiet\n" >> /etc/pam.d/gdm-password.tmp
  mv -f /etc/pam.d/gdm-password.tmp /etc/pam.d/gdm-password \
  2>&1 | tee -a $LOGFILE
else
  #echo "Search string not found...appending"
  printf "auth\trequired\tpam_succeed_if.so user != root quiet\n" >> /etc/pam.d/gdm-password
fi

chown root:root /etc/pam.d/gdm-password 2>&1 | tee -a $LOGFILE
chmod 644 /etc/pam.d/gdm-password 2>&1 | tee -a $LOGFILE

echo "-- END: Disabling root login on console" 2>&1 | tee -a $LOGFILE

#---------------------------------------------------
# Non-audit from DLB 2.x
# Installs consent banner

echo "-- START: Installing and configuring consent banners" 2>&1 | tee -a $LOGFILE

if [ -d /etc/gdm/PostLogin ]; then
   cp -f $CURRENT_DIR/CONSENT/Default /etc/gdm/PostLogin/Default  2>&1 | tee -a $LOGFILE
   chmod 755 /etc/gdm/PostLogin/Default  2>&1 | tee -a $LOGFILE
   chown root:root /etc/gdm/PostLogin/Default  2>&1 | tee -a $LOGFILE
fi

echo "-- END: Installing and configuring consent banners" 2>&1 | tee -a $LOGFILE
...
case ${1} in
  ...
  *)
	doUpdate 
	echo "$CURDATE | $OSVERSION | $VERSION | Completed refresh of affiliate security for DIA Desktop - doUpdate" >> /etc/.icgc-cm.log
	exit 0
    ;;
esac
