...
script_V38545-describe:
  cmd.script:
    - source: salt://ash-linux/STIGbyID/cat3/files/V38545.sh

# Monitoring of SELinux DAC config
{% if grains['cpuarch'] == 'x86_64' %}
# ...for unprivileged users
  {% if salt['file.search']('/etc/audit/audit.rules', '-a always,exit -F arch=b64 -S chown -F auid>=500 -F auid!=4294967295 -k perm_mod') %}
file_V38545-auditRules_selDACusers:
  cmd.run:
    - name: 'echo "Appropriate audit rule already in place"'
  {% elif salt['file.search']('/etc/audit/audit.rules', ' chown -F auid>=500 ') %}
file_V38545-auditRules_selDACusers:
  file.replace:
    - name: '/etc/audit/audit.rules'
    - pattern: '^.* chown -F auid>=500 .*$'
    - repl: '-a always,exit -F arch=b64 -S chown -F auid>=500 -F auid!=4294967295 -k perm_mod'
  {% else %}
file_V38545-auditRules_selDACusers:
  file.append:
    - name: '/etc/audit/audit.rules'
...
# ...for root user
  {% if salt['file.search']('/etc/audit/audit.rules', '-a always,exit -F arch=b64 -S chown -F auid=0 -k perm_mod') %}
file_V38545-auditRules_selDACroot:
  cmd.run:
    - name: 'echo "Appropriate audit rule already in place"'
  {% elif salt['file.search']('/etc/audit/audit.rules', ' chown .*auid=0 ') %}
file_V38545-auditRules_selDACroot:
  file.replace:
    - name: '/etc/audit/audit.rules'
    - pattern: '^.* chown .*auid=0 .*$'
    - repl: '-a always,exit -F arch=b64 -S chown -F auid=0 -k perm_mod'
  {% else %}
file_V38545-auditRules_selDACroot:
  file.append:
    - name: '/etc/audit/audit.rules'
