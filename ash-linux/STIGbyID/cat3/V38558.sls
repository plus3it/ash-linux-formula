...
script_V38558-describe:
  cmd.script:
    - source: salt://ash-linux/STIGbyID/cat3/files/V38558.sh

# Monitoring of SELinux DAC config
{% if grains['cpuarch'] == 'x86_64' %}
# ...for unprivileged users
  {% if salt['file.search']('/etc/audit/audit.rules', '-a always,exit -F arch=b64 -S lchown -F auid>=500 -F auid!=4294967295 -k perm_mod') %}
...
file_V38558-auditRules_selDACusers:
  file.append:
    - name: '/etc/audit/audit.rules'
    - text:
      - '# Monitor for SELinux DAC changes (per STIG-ID V-38558)'
      - '-a always,exit -F arch=b64 -S lchown -F auid>=500 -F auid!=4294967295 -k perm_mod'
  {% endif %}

# ...for root user
  {% if salt['file.search']('/etc/audit/audit.rules', '-a always,exit -F arch=b64 -S lchown -F auid=0 -k perm_mod') %}
file_V38558-auditRules_selDACroot:
  cmd.run:
    - name: 'echo "Appropriate audit rule already in place"'
  {% elif salt['file.search']('/etc/audit/audit.rules', ' lchown .*auid=0 ') %}
file_V38558-auditRules_selDACroot:
  file.replace:
    - name: '/etc/audit/audit.rules'
    - pattern: '^.* lchown .*auid=0 .*$'
    - repl: '-a always,exit -F arch=b64 -S lchown -F auid=0 -k perm_mod'
  {% else %}
file_V38558-auditRules_selDACroot:
  file.append:
    - name: '/etc/audit/audit.rules'
