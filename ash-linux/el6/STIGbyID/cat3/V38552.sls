...
{%- set usertypes = {
    'selDACusers' : { 'search_string' : ' fchown -F auid>=500 ',
                      'rule' : '-a always,exit -F arch=b64 -S fchown -F auid>=500 -F auid!=4294967295 -k perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S fchown -F auid>=500 -F auid!=4294967295 -k perm_mod',
                    },
    'selDACroot'  : { 'search_string' : ' fchown .*auid=0 ',
                      'rule' : '-a always,exit -F arch=b64 -S fchown -F auid=0 -k perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S fchown -F auid=0 -k perm_mod',
                    },
} %}
