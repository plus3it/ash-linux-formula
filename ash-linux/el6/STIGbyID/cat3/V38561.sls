...
{%- set usertypes = {
    'selDACusers' : { 'search_string' : ' lsetxattr -F auid>=500 ',
                      ...
                      'rule32' : '-a always,exit -F arch=b32 -S lsetxattr -F auid>=500 -F auid!=4294967295 -k perm_mod',
                    },
    'selDACroot'  : { 'search_string' : ' lsetxattr .*auid=0 ',
                      'rule' : '-a always,exit -F arch=b64 -S lsetxattr -F auid=0 -k perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S lsetxattr -F auid=0 -k perm_mod',
                    },
} %}
