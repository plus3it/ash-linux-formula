...
{%- set usertypes = {
    'selDACusers' : { 'search_string' : ' chown -F auid>' + sysuserMax + ' ',
                      'rule' : '-a always,exit -F arch=b64 -S chown -F auid>' + sysuserMax + ' -F auid!=4294967295 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S chown -F auid>' + sysuserMax + ' -F auid!=4294967295 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                    },
    'selDACroot'  : { 'search_string' : ' chown -F auid=0 ',
                      'rule' : '-a always,exit -F arch=b64 -S chown -F auid=0 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S chown -F auid=0 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                    },
} %}
