...
{%- set usertypes = {
    'selDACusers' : { 'search_string' : ' fchownat -F auid>' + sysuserMax + ' ',
                      'rule' : '-a always,exit -F arch=b64 -S fchownat -F auid>' + sysuserMax + ' -F auid!=4294967295 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S fchownat -F auid>' + sysuserMax + ' -F auid!=4294967295 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                    },
    'selDACroot'  : { 'search_string' : ' fchownat -F auid=0 ',
                      'rule' : '-a always,exit -F arch=b64 -S fchownat -F auid=0 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                      'rule32' : '-a always,exit -F arch=b32 -S fchownat -F auid=0 -F subj_role=unconfined_u:unconfined_r:unconfined_t:s0-s0:c0.c1023 -F key=perm_mod',
                    },
} %}
