...
{% if grains['cpuarch'] == 'x86_64' %}
  {% if salt['file.search']('/etc/audit/audit.rules', '-a always,exit -F arch=b64 -S lchown -F auid>=500 -F auid!=4294967295 -k perm_mod -a always,exit -F arch=b64 -S lchown -F auid=0 -k perm_mod') %}
...
file_V38558-auditRules_selDAC:
  ...
  - pattern: '^.* lchown .*$'
  - repl: '-a always,exit -F arch=b64 -S lchown -F auid>=500 -F auid!=4294967295 -k perm_mod -a always,exit -F arch=b64 -S lchown -F auid=0 -k perm_mod'
  {% else %}
file_V38558-auditRules_selDAC:
  file.append:
  - name: '/etc/audit/audit.rules'
